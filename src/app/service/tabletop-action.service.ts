...
import { common, player1, player2 } from 'json/game/game.json';
...
export class TabletopActionService {
  ...
  makeDefaultTable() {
    ...
    let testBgFile_back: ImageFile = null;
    let bgFileContext = ImageFile.createEmpty('testTableBackgroundImage_image').toContext();
    let bgFileContext_back = ImageFile.createEmpty('testTableBackgroundImage_image_back').toContext();
    ...
    let url: string = './assets/images/raigo/koma/雷.jpg';
    let back: string = './assets/images/raigo/koma/ura.jpg';
    if (!ImageStorage.instance.get(url)) {
      ImageStorage.instance.add(url);
    }
    if (!ImageStorage.instance.get(back)) {
