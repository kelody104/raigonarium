...
export class GameTableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private viewPotisonX: number = -450;
  private viewPotisonY: number = -100;
  private viewPotisonZ: number = -850;

  private viewRotateX: number = 25; //30
  private viewRotateY: number = 0;
  private viewRotateZ: number = -720;
  ...
  ngOnInit() {
    ...
    this.commonActionService.setViewRotateZ(this.viewRotateZ);
  }
  ...
  ngAfterViewInit() {
    ...
    // 例：player1 左側、player2 右側に同じサイズで置く
    this.gorgeService.init(
      { x: 60, y: 730, width: 385, height: 490 },  // player1 側の峡谷
      { x: 2005, y: 180, width: 385, height: 490 },  // player2 側の峡谷（値は盤面に合わせて調整）
    );
