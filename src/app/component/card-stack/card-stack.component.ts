...
export class CardStackComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  get topCard(): Card | null {
    return this.cardStack && this.cardStack.topCard
      ? this.cardStack.topCard
      : null;
  }

  // ★観戦モードでは「山札のカードの表面」を必ず表示する
  get imageFile(): ImageFile {
    const top = this.topCard;

    // 観戦モード中 & 山札にカードがある場合は、常に topCard の表画像
    if (this.isSpectator && top && top.frontImage) {
      return this.imageService.getSkeletonOr(top.frontImage);
    }

    // 通常時は従来どおり山札の imageFile を使う
  ...
  set isLocked(v: boolean) { this.cardStack.isLocked = v; }
  ...
  onDoubleClick() {
    this.ngZone.run(() => {
      (this.drawCardsToDrawSlots(1) > 0); SoundEffect.play(PresetSound.cardDraw);
    });
  }
  ...
  private makeContextMenu(): ContextMenuAction[] {
    ...
    actions.push({
      name: '１枚引く', action: () => {
        if (this.drawCardsToDrawSlots(1) > 0) {
          SoundEffect.play(PresetSound.cardDraw);
        }
      }
    });
    actions.push({
      name: '２枚引く', action: () => {
        if (this.drawCardsToDrawSlots(2) > 0) {
          SoundEffect.play(PresetSound.cardDraw);
        }
      }
    });
    actions.push({
      name: '３枚引く（浄）', action: () => {
        if (this.drawCardsToDrawSlots(3) > 0) {
          SoundEffect.play(PresetSound.cardDraw);
        }
      }
    });
    actions.push({
      name: '４枚引く（浄）', action: () => {
        if (this.drawCardsToDrawSlots(4) > 0) {
          SoundEffect.play(PresetSound.cardDraw);
        }
      }
    });
  ...
  private drawCardsToDrawSlots(count: number): number {
    if (count <= 0) return 0;

    const player = this.commonActionService.getCurrentPlayer();
    const drawPositions = this.commonActionService.getDrawPositions(player);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const pos = drawPositions[i] ?? drawPositions[drawn] ?? null;
